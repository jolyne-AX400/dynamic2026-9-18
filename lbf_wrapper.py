...
class LBFWrapper(object):
    ...
    def get_comm_info(self, info=None):
        ...
        merged["action_mask"] = self._get_action_mask()
        merged["alive_mask"] = np.ones(self.nagents, dtype=np.float64)
        return merged

    def _get_action_mask(self):
        """Return the legal LBF actions for every agent as a binary mask.

        LBF converts illegal actions (most importantly LOAD when no food is in
        range) to NONE.  Without masking, policy-gradient training can assign
        most probability to an illegal LOAD action and appear to learn while
        the agents actually remain stationary.
        """
        mask = np.zeros((self.nagents, self.num_actions), dtype=np.float64)
        players = getattr(self._base_env, "players", None)
        valid_actions = getattr(self._base_env, "_valid_actions", None)

        if players is None or not isinstance(valid_actions, dict):
            mask.fill(1.0)
            return mask

        for agent_index, player in enumerate(players[:self.nagents]):
            actions = valid_actions.get(player, None)
            if actions is None:
                mask[agent_index].fill(1.0)
                continue

            for action in actions:
                action_index = getattr(action, "value", action)
                action_index = int(action_index)
                if 0 <= action_index < self.num_actions:
                    mask[agent_index, action_index] = 1.0

            # NONE (action 0) is the safe fallback if an unexpected LBF state
            # exposes an empty valid-action list.
            if mask[agent_index].sum() == 0:
                mask[agent_index, 0] = 1.0
