...
class MAGIC(nn.Module):
    ...
    def forward(self, x, info={}):
        ...
        action_out = []
        policy_input = torch.cat((h, c), dim=-1)
        for action_head in self.action_heads:
            action_logits = action_head(policy_input)
            action_logits = self.apply_action_mask(action_logits, info)
            action_out.append(F.log_softmax(action_logits, dim=-1))

        return action_out, value_head, (hidden_state.clone(), cell_state.clone())

    def apply_action_mask(self, action_logits, info):
        """Mask actions that the current environment state cannot execute.

        Environments without an ``action_mask`` keep the original behavior.
        The finite negative value is deliberate: it produces zero probability
        for invalid actions without introducing ``0 * -inf`` NaNs in entropy.
        """
        ...
        if action_mask.dim() != 3:
            raise ValueError(
                'action_mask must have shape [n, actions] or [batch, n, actions], got {}'.format(
                    tuple(action_mask.size())
                )
            )
        if action_mask.size(1) != action_logits.size(1) or action_mask.size(2) != action_logits.size(2):
            raise ValueError(
                'action_mask shape {} is incompatible with action logits {}'.format(
                    tuple(action_mask.size()), tuple(action_logits.size())
                )
            )
        if action_mask.size(0) == 1 and action_logits.size(0) > 1:
            action_mask = action_mask.expand(action_logits.size(0), -1, -1)
        elif action_mask.size(0) != action_logits.size(0):
            raise ValueError(
                'action_mask batch size {} does not match policy batch size {}'.format(
                    action_mask.size(0), action_logits.size(0)
                )
            )

        valid_rows = action_mask.sum(dim=-1, keepdim=True) > 0
        action_mask = torch.where(valid_rows, action_mask, torch.ones_like(action_mask))
        return action_logits.masked_fill(action_mask <= 0, -1e9)
