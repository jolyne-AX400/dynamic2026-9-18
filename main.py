...
parser.add_argument('--lbf_env_id',
                    default='Foraging-2s-8x8-2p-2f-coop-v2',
                    type=str,
                    help='registered lb-foraging Gym environment id')
...
def print_training_panel(stat, epoch, num_episodes, epoch_time):
    ...
    if 'food_collected' in stat:
        print(format_metric_line('Food-Collected', stat.get('food_collected', 0.0), '{:.4f}'))
    if 'food_completion' in stat:
        print(format_metric_line('Food-Completion', stat.get('food_completion', 0.0), '{:.4f}'))
    if 'add_rate' in stat:
...
def main():
    ...
    log['food_collected'] = LogField(list(), True, 'epoch', 'num_episodes')
    log['food_completion'] = LogField(list(), True, 'epoch', 'num_episodes')
    ...
    elif args.env_name == 'lbf':
        safe_env_id = args.lbf_env_id.replace('/', '_').replace('\\', '_')
        model_dir = model_dir / safe_env_id / args.comm_arch / ('seed_{}'.format(args.seed))
    ...
    def export_metric_window(start_epoch, end_epoch):
        ...
        fieldnames = [
            ...
            'food_collected', 'food_completion',
